...
class JarvisParser:
            ...
            for line in file:
                ...
                if len(splitLine) > 0:
                    if CategoryDesc.has_value(splitLine[0]):
                        ...
                        raise \
                            Exception("Parsing [L:{}] | The argument isn't valid [got '{}']"
                                      .format(countLine, splitLine[0]))
    ...
    def __parsing_META(self, splitLine: [str], countLine: int):
        if not self.is_meta_defined:
            if self.infos[CategoryDesc.META] == self.infos[CategoryDesc.INPUT] \
                ...
                raise \
                    Exception("Parsing ['{}'|L:{}] | The meta character is already used in input alphabet or in the output alphabet"
                              .format(CategoryDesc.NB_STATES, countLine))
            elif len(splitLine) != 2:
                raise \
                    Exception("Parsing ['{}'|L:{}] | The argument size of the line didn't correspond [expected {} - got {}]"
            ...
            raise \
                Exception("Parsing ['{}'|L:{}] | This type of information has already been given."
                          .format(CategoryDesc.NB_STATES, countLine))
    ...
    def __parsing_INPUT(self, splitLine: [str], countLine: int):
        if not self.is_input_alphabet_set:
            if len(splitLine) != 2:
                ...
                raise \
                    Exception("Parsing ['{}'|L:{}] | The meta character doesn't have to be in the input alphabet."
                              .format(CategoryDesc.NB_STATES, countLine, 2, len(splitLine)))
            else:
    ...
    def __parsing_OUTPUT(self, splitLine: [str], countLine: int):
        if not self.is_output_alphabet_set:
            if len(splitLine) != 2:
                ...
                raise \
                    Exception("Parsing ['{}'|L:{}] | The meta character doesn't have to be in the output alphabet."
                              .format(CategoryDesc.NB_STATES, countLine, 2, len(splitLine)))
            else:
    ...
    def __parsing_NB_STATES(self, splitLine: [str], countLine: int):
        if len(splitLine) == 2:
            ...
            if not str(nbStates).isnumeric():
                raise \
                    Exception("Parsing ['{}'|L:{}] | The given value isn't numeric [expected '{}' - got '{}']"
                              .format(CategoryDesc.NB_STATES.value, countLine, "0|[0-9][0-9]*", nbStates))

            nbStates = int(splitLine[1])
            if nbStates <= 0:
                raise \
                    Exception("Parsing ['{}'|L:{}] | The given value cannot be used as a valid argument [expected '{}' - got '{}']"
                              .format(CategoryDesc.NB_STATES.value, countLine, "> 0", nbStates))

            maxValue = max(max(self.infos[CategoryDesc.INIT]), max(self.infos[CategoryDesc.FINAL] or [0]))
            if maxValue >= int(nbStates):
                raise \
                    Exception("Parsing ['{}'|L:{}] | The given argument surpass the number of state already know by the program [expected '< {}' - got '{}']"
                              .format(CategoryDesc.NB_STATES.value, countLine, maxValue, nbStates))
            ...
            raise \
                Exception("Parsing ['{}'|L:{}] | The argument size of the line didn't correspond [expected '{}' - got '{}']"
                          .format(CategoryDesc.NB_STATES.value, countLine, 2, len(splitLine)))
    ...
    def __parsing_INIT(self, splitLine: [str], countLine: int):
        if not len(splitLine) >= 2:
            raise \
                Exception("Parsing ['{}'|L:{}] | The argument size of the line didn't correspond [expected '{}' - got '{}']"
                          .format(CategoryDesc.INIT.value, countLine, "> 2", len(splitLine)))
        ...
        for arg in splitLine[1:]:
            if not str(arg).isnumeric():
                raise \
                    Exception("Parsing ['{}'|L:{}] | The given value isn't numeric [expected '{}' - got '{}']"
                              .format(CategoryDesc.INIT.value, countLine, "0|[0-9][0-9]*", arg))

            if int(arg) in self.infos[CategoryDesc.INIT]:
                raise \
                    Exception("Parsing ['{}'|L:{}] | The given argument already exists on the variable [got '{}']"
                              .format(CategoryDesc.NB_STATES.value, countLine, arg))

            if 0 <= self.infos[CategoryDesc.NB_STATES] <= int(arg):
                raise \
                    Exception("Parsing ['{}'|L:{}] | The given argument surpass the number of state already know by the program [expected '< {}' - got '{}']"
                              .format(CategoryDesc.NB_STATES.value, countLine, self.infos[CategoryDesc.NB_STATES], arg))
    ...
    def __parsing_FINAL(self, splitLine: [str], countLine: int):
        if not len(splitLine) >= 2:
            raise \
                Exception("Parsing ['{}'|L:{}] | The argument size of the line didn't correspond [expected '{}' - got '{}']"
                          .format(CategoryDesc.FINAL.value, countLine, "> 2", len(splitLine)))

        for arg in splitLine[1:]:
            if not str(arg).isnumeric():
                raise \
                    Exception("Parsing ['{}'|L:{}] | The given value isn't numeric [expected '{}' - got '{}']"
                              .format(CategoryDesc.FINAL.value, countLine, "0|[0-9][0-9]*", arg))

            if int(arg) in self.infos[CategoryDesc.FINAL]:
                raise \
                    Exception("Parsing ['{}'|L:{}] | The given argument already exists on the variable [got '{}']"
                              .format(CategoryDesc.FINAL.value, countLine, arg))

            if 0 <= self.infos[CategoryDesc.NB_STATES] <= int(arg):
                raise \
                    Exception("Parsing ['{}'|L:{}] | The given argument surpass the number of state already know by the program [expected '< {}' - got '{}']"
                              .format(CategoryDesc.FINAL.value, countLine, self.infos[CategoryDesc.NB_STATES], arg))
    ...
    def __parsing_TRANSITIONS(self, splitLine: [str], countLine: int):
        if 4 < len(splitLine) > 5:
            raise \
                Exception("Parsing ['{}'|L:{}] | The argument size of the line didn't correspond [expected '{}' - got '{}']"
                          .format(CategoryDesc.TRANSITIONS.value, countLine, "4 <-> 5", len(splitLine)))
        ...
        if self.infos[CategoryDesc.NB_STATES] <= 0:
            raise \
                Exception("Parsing ['{}'|L:{}] | The number of state isn't correct or inexistant [expected '{}' - got '{}']"
                          .format(CategoryDesc.TRANSITIONS.value, countLine, "{} > 0".format(CategoryDesc.NB_STATES.value), self.infos[CategoryDesc.NB_STATES]))

        if splitLine[1].isnumeric() and splitLine[3].isnumeric():
            if int(splitLine[1]) > (self.infos[CategoryDesc.NB_STATES]):
                raise \
                    Exception("Parsing ['{}'|L:{}] | State numbers are not supposed to be equal or more than the number of states : expected less than {}, got {}"
                              .format(CategoryDesc.TRANSITIONS.value, countLine, self.infos[CategoryDesc.NB_STATES], splitLine[1]))
            if int(splitLine[3]) > (self.infos[CategoryDesc.NB_STATES]):
                raise \
                    Exception("Parsing ['{}'|L:{}] | State numbers are not supposed to be equal or more than the number of states : expected less than {}, got {}"
                              .format(CategoryDesc.TRANSITIONS.value, countLine, self.infos[CategoryDesc.NB_STATES], splitLine[3]))
        else:
            raise \
                Exception("Parsing ['{}'|L:{}] | Argument type is not of the expected type (int)"
        ...
        if (splitLine[2].strip("'") not in self.infos[CategoryDesc.INPUT]
            ...
            raise \
                Exception("Parsing ['{}'|L:{}] | Transition has a character unexpected, it needs to be an input alphabet character or the meta character"
                          .format(CategoryDesc.TRANSITIONS.value, countLine))
        ...
        if self.infos[CategoryDesc.OUTPUT]:
            if (splitLine[4].strip("'") not in self.infos[CategoryDesc.OUTPUT]
                ...
                raise \
                    Exception("Parsing ['{}'|L:{}] | Transition has a character unexpected, it needs to be an output alphabet character or the meta character"
                              .format(CategoryDesc.TRANSITIONS.value, countLine))
